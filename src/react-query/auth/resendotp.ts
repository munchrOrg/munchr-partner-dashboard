  ...
  return useMutation({
    mutationFn: async ({ partnerId, email }: { partnerId: string; email: string }) => {
      const resp = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}api/partner/otp/resend-email`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ entityId: partnerId, entityType: 'partner', email }),
        }
      );
      if (!resp.ok) {
...
export function useResendPhoneOtp() {
  return useMutation({
    mutationFn: async ({ partnerId, phone }: { partnerId: string; phone: string }) => {
      const resp = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}api/partner/otp/resend-phone`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ entityId: partnerId, entityType: 'partner', phone }),
        }
      );
      if (!resp.ok) {
